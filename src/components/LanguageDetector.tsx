...
const LanguageDetector: React.FC<LanguageDetectorProps> = ({ inputText = "", translatedText = "", setDetectedLang }) => {
  ...
  const detectLanguage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const bestMatch = results[0]; // Highest confidence
      if (bestMatch.confidence >= 0.1) {
        console.log(` Detected language: ${bestMatch.detectedLanguage} (Confidence: ${bestMatch.confidence})`);
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={() => detectLanguage(inputText)} className="bg-[#480468] text-white px-4 py-2 rounded-md" disabled={loading}>
